...
const severityConfig: Record<string, { bg: string; text: string; border: string; dot: string }> = {
  "高": { bg: "rgba(239,68,68,0.08)", text: "var(--red)", border: "rgba(239,68,68,0.2)", dot: "bg-[var(--red)]" },
  "中": { bg: "rgba(245,158,11,0.08)", text: "var(--amber)", border: "rgba(245,158,11,0.2)", dot: "bg-[var(--amber)]" },
  "低": { bg: "rgba(34,197,94,0.06)", text: "var(--green)", border: "rgba(34,197,94,0.15)", dot: "bg-[var(--green)]" },
};

export function RiskAnalysis({ data, ticker, riskInsight }: Props) {
  const riskLevel = riskInsight?.overall_risk_level || "中";
  ...
  const levelCfg = severityConfig[riskLevel] || severityConfig["中"];
        ...
        {riskItems.length > 0 ? (
          <div className="p-3 space-y-2">
            {riskItems.map((item, i) => {
              const sev = severityConfig[item.severity] || severityConfig["中"];
              return (
                <motion.div
                  key={i}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
              {data.map((exp, i) => (
                <div
                  ...
                  style={{
                    background:
                      exp.level === "危险" ? "rgba(239,68,68,0.06)" :
                      exp.level === "高" ? "rgba(245,158,11,0.06)" :
                      "rgba(59,130,246,0.04)",
                    border: `1px solid ${
                      exp.level === "危险" ? "rgba(239,68,68,0.15)" :
                      exp.level === "高" ? "rgba(245,158,11,0.15)" :
                      "rgba(59,130,246,0.1)"
                    }`,
                  }}
                >
